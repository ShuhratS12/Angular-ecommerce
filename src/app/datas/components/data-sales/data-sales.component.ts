...
import { SalesData } from '../../models/datas.model';
...
export class DataSalesComponent implements OnInit {
  displayedColumns = ['reference', 'articles', 'salesNumber', 'taxes', 'incomeExclT', 'incomeInclT'];
  displayedFooterColumns = ['prev', 'showmore', 'next'];
  items: SalesData[];
  tempItems: SalesData[];
  searchTotalItems: SalesData[];
  ...
  constructor(
    ...
    private storeService: StoreService) { 
      this.tempItems = this.ordersService.getSalesData();
  ...
  getTotalSalesNumber() {
    return this.searchTotalItems.map(t => t.salesNumber).reduce((acc, value) => acc + value, 0);
  }
  getTotalTaxes() {
